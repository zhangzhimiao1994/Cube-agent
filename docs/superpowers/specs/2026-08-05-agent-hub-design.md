...
每个模型 Deployment 配置凭证引用、`quota_scope_id`、`max_concurrency`、安全利用率、预留容量、RPM、TPM、请求超时、优先级和费用信息。`quota_scope_id` 表示供应商实际计算额度的范围，可能是 API Key、账号、项目或中转站租户。支持并发的配额池可以把 `max_concurrency` 设置为大于 `1`，由多个 Agent 共享；只能串行调用的配额池设置为 `1`。

同一配额范围内的多个 API Key 共享一个容量信号量，不能重复计算容量。例如 DeepSeek 官方并发额度按账号统计，同账号的多个 Key 不会增加并发；这类凭证必须指向相同 `quota_scope_id`。需要增加容量时，应配置真正独立的配额账号/项目、申请供应商扩容或增加其他供应商，而不是复制同账号密钥。

每次模型调用必须先从 ModelGateway 获取有过期时间的配额池容量租约，直到非流式响应完成或流式连接关闭后才释放。网关使用 Redis 原子信号量和速率计数器，按健康状态、剩余并发槽、RPM/TPM、权重和费用选择最合适的 Deployment。多个 Agent 不接触具体密钥，也不能绕过租约直接调用模型。

系统默认 `target_utilization=0.8`，有效并发为配置上限乘以安全利用率并至少保留策略规定的容量余量；低并发池至少保留一个可用槽。管理员可在受控范围内调整，但自动调节不得超过安全水位。网关同时使用令牌桶限制 RPM/TPM，并根据队列长度、P95 延迟、`429` 和 `503` 主动降速，避免跑满供应商配额。

所有槽位被占用时，请求默认进入有界等待队列；超过配置的等待时间后，才按回退链切换到备用 API 或模型。如果任务禁止模型切换，则保持排队直到任务期限，并在飞书/Web 显示等待原因。用户取消、Worker 崩溃或租约过期时必须回收容量。

系统使用不可逆密钥指纹识别重复凭证；同一 API Key 被重复添加时不会被计算为额外容量，同一 `quota_scope_id` 的不同 Key 也不会叠加配额。Web 管理台提供渐进、非饱和的并发探测，结果只作为配置建议，因为供应商和中转站限制可能动态变化。运行期遇到 `429`、超时或服务端限流时自动降低有效并发并进入冷却，稳定后逐步恢复到安全水位而非配置上限。
...
- 并发 API 可由多个 Agent 共享；串行配额池使用单槽；只有真正独立的配额范围才能叠加并行容量；系统保留安全余量，满载前进行背压排队，超时后才降级。
