...
**Pre-release fingerprint v2 decision:** Scheme v2 was introduced before the first Task 5 release. Remote `main` remained at `d923fcd`, where neither `SecretRow` nor encrypted-secret persistence existed, so there are no production or target-environment rows using an earlier experimental fingerprint algorithm. Any rows generated by Task 5 experimental commits before `93f00f1` are pre-release incompatible data; this explicitly includes rows from `135caf1` that used an explicit fingerprint root. Such rows must either be cleared or decrypted with the corresponding old cipher and rewritten with the current implementation before upgrading. The `0004_encrypted_secrets` schema does not automatically make those fingerprint values compatible.
