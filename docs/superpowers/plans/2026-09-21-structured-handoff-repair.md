...
- [ ] Reverse reviewer exception soft-skip tests for timeout, capacity, empty/invalid
...
- [ ] Run `python -m pytest tests/unit/runtime/crew/test_structured_handoff_repair.py tests/unit/runtime/crew/test_adapter_failure_reason.py -k 'structured or handoff or wrapped or reviewer' -q`.
...
- [ ] Replace skipped/timeout_skipped+approve with explicit failed/unverified review
...
- [ ] Add reviewer-invalid -> genuine approve/revise/reject correction tests when
...
- [ ] Add no-authorized-waiver failure tests. Only exercise waiver success if an
  existing trusted policy supports it; keep its audit distinct from model approval.
- [ ] Observe red with `python -m pytest tests/unit/runtime/crew/test_structured_handoff_repair.py -k 'correction or framework_raw or reviewer' -q`.
...
- [ ] For reviewer correction use purpose=review and its existing review cursor;
...
- [ ] Assert worker correction then reviewer failure cannot make a second
...
- [ ] Cover review replay against changed candidate ID/digest, legacy skipped+
...
- [ ] Repeat accounting/cancel/restore tests for reviewer original and correction:
...
- [ ] Add PG reviewer-failure and worker-used-slot/reviewer-invalid cases: no
