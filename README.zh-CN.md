...
- 在 Web 或已接入通道里和主 Agent 对话，继续历史会话，从历史上下文新建分支，上传附件，并在对话内启用 Vibe Coding。
...
历史会话行提供按原思路新建分支的入口；分支引用激活后，输入区只显示明确的取消引用控件，不再要求每条消息手动打开 Handoff。Vibe Coding 仍是对话内代码协作能力，开启后有明显选中状态，并可在发送前关闭。正在运行的对话可以在输入区停止生成；对话里检测出的计划任务或进化任务提案，也可以在创建持久记录前取消。
