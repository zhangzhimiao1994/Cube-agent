...
- 创建一次性计划任务或 cron 周期任务；对话里识别到“每天/每周/提醒/日程”这类需求时，先给出计划方案，再由用户确认创建。
...
Handoff 和 Vibe Coding 是两个独立按钮，可以同时打开，也可以在发送前取消。Handoff 用于引用历史会话；Vibe Coding 是对话内代码协作能力，不作为独立系统模块。
...
如果用户在对话里提出计划任务，例如“每天 9 点提醒我填报表”或“每周一生成周报”，系统会先生成计划方案，用户确认后才写入计划任务。
...
飞书用户可以在消息开头使用交互指令，不需要猜系统会走哪种模式：
...
管理员可以在飞书通道的 `FEISHU_COMMAND_ALIASES` 字段里配置自定义别名，例如 `方案=//派单, 代码=//vi, 菜单=//帮助`。保存后，通道页面和飞书帮助回复都会显示当前生效的别名。
...
Hermes 学习独立于对话页面。学习记录可以单条确认、单条删除、批量确认和批量删除。
