...
Operations use configured command allowlists and adapter records. Local Linux server commands can run through the bundled adapter. Remote adapters can also perform bounded `file_read` operations without an argv command when `OPENCLAW_ADAPTER_ALLOWED_FILE_ROOTS_JSON` is configured with explicit absolute roots; output is capped by `OPENCLAW_ADAPTER_FILE_READ_LIMIT_BYTES`. Windows, Linux desktop, macOS, screen, and filesystem targets should be connected with dedicated credentials and least privilege. Every remote adapter must expose `/v1/openclaw/health` with its platform and supported capabilities; Agent Hub checks that health response before execution so unsupported desktop, screen, or file operations are not treated as available.
