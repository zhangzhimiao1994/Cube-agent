...
function formatDetailPayloadDisplayValue(key: string, value: unknown): string {
  if (key === "repair_action" || key === "repair_kind" || key === "remediation_action") {
    return repairActionLabel(formatDetailValue(value));
  }
  if (key === "failure_kind") {
...
function safeActionTargetRows(event: RunEvent): DetailProcessRow[] {
  const rows: DetailProcessRow[] = [];
...
function detailCardActionTarget(card: DetailProcessCard) {
  ...
  const primaryTarget =
    detailCardRowValue(card, /^(关联文件|文件|路径|工作区文件|命令|目标)$/) ||
    detailCardRowValue(card, /command|path|file|target/i);
...
function detailDiagnosticFromApi(
  ...
  const detail = [repairFailureKindLabel(diagnostic.failure_kind), statusCode, detailDiagnosticReason(diagnostic)]
    .filter(Boolean)
  ...
  detail.events.forEach((event) => {
    ...
    if (event.kind === "step.retrying") {
      ...
      pushUniqueIntent(intents, {
        ...
        meta: [
          attempt ? `第 ${attempt} 次` : "",
          repairFailureKindLabel(safeIntentIdentifier(event, ["failure_kind"])) || safeIntentIdentifier(event, ["status"]),
          replaySafetyLabel(event.payload.replay_safe),
        ].filter(Boolean),
        tone: "retry",
      });
    ...
    if (isRepairIntentEvent(event)) {
      const rawRepairAction = safeIntentIdentifier(event, ["repair_action", "repair_kind", "remediation_action"]);
      const repairAction = repairActionLabel(rawRepairAction) || repairActionLabel(safeDiagnosticIdentifier(event.action, "")) || "修复方案";
      const eventAction = repairActionLabel(safeDiagnosticIdentifier(event.action, ""));
      pushUniqueIntent(intents, {
        id: `${detail.id}-intent-repair-${event.step_id ?? "repair"}-${event.sequence}`,
        label: "修复意图",
        title: repairAction,
