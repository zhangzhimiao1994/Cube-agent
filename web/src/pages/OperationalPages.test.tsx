...
const pluginAdapterDescriptors: PluginAdapterDescriptor[] = [
  {
    id: "http_json",
    name: "HTTP JSON",
...
async function openAgentWorkbench(user: ReturnType<typeof userEvent.setup>, stream: HTMLElement) {
  const existing = screen.queryByRole("dialog", { name: "Agent 工作席详情" });
  if (existing) return existing;
  await user.click(within(stream).getByRole("button", { name: /Agent 工作席/ }));
  return screen.findByRole("dialog", { name: "Agent 工作席详情" });
}

function drawerBackdrop(drawer: HTMLElement) {
  const backdrop = drawer.parentElement;
  expect(backdrop).not.toBeNull();
  expect(backdrop?.classList.contains("process-drawer-backdrop")).toBe(true);
...
async function openWorkbenchView(
  ...
  name: "助手总览" | "调度讨论" | "实际动作" | "文件" | "终端" | "结果" | "修复异常" | "动作与文件",
) {
  const compactName =
    name === "调度讨论" || name === "助手总览"
      ? "助手总览"
      : name === "动作与文件"
        ? "动作与文件"
        : "动作与文件";
  const compactButton = within(workbench).queryByRole("button", { name: compactName });
  const legacyButton = within(workbench).queryByRole("button", { name: name === "动作与文件" ? "实际动作" : name });
  await user.click(compactButton ?? legacyButton ?? within(workbench).getByRole("button", { name: compactName }));
  return workbench;
}

function queryWorkbenchActionButton(workbench: HTMLElement, name: RegExp) {
  const actionLists = Array.from(workbench.querySelectorAll(".agent-cluster-actions"));
  for (const actionList of actionLists) {
    const match = within(actionList as HTMLElement).queryAllByRole("button", { name })[0];
    if (match) return match;
  }
  return within(workbench).queryAllByRole("button", { name })[0] ?? null;
}

function getWorkbenchActions(workbench: HTMLElement) {
  return within(workbench).queryByLabelText("过程轨迹")
    ?? within(workbench).getByLabelText("Agent 工作席动作");
}
...
async function openWorkbenchProcessButton(
  ...
  const overviewMatch = queryWorkbenchActionButton(workbench, name);
  if (overviewMatch) return overviewMatch;
  await openWorkbenchView(user, workbench, "动作与文件");
  const actionMatch = queryWorkbenchActionButton(workbench, name);
...
describe("operational management pages", () => {
  ...
  beforeEach(() => {
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (path === "/api/v1/admin/project-workspaces" && method === "GET") {
          return jsonResponse(visibleProjectWorkspaces);
        }
        if (path === "/api/v1/admin/project-workspaces" && method === "POST") {
  ...
  async function expandRunConfigDetails(user: ReturnType<typeof userEvent.setup>) {
  ...
  it("renders run detail as a Vibe Engineer debugging summary", async () => {
    ...
    const workbenchActions = getWorkbenchActions(actionDrawer);
  ...
  it("keeps unknown run detail event kinds debuggable without showing raw codes in the summary", async () => {
    ...
    const workbenchActions = getWorkbenchActions(workbenchDrawer);
    ...
    const evidenceDetail = await openProcessDetailGroup(user, workbenchDrawer, "证据");
  ...
  it("keeps reference workflows advisory while main-agent routing stays automatic", async () => {
    ...
    expect(screen.queryByText("1 · 会话")).toBeNull();
    expect(screen.queryByText("2 · 对话")).toBeNull();
    expect(screen.queryByText("3 · 设置 / 详情")).toBeNull();
    const conversationPage = screen.getByRole("heading", { name: "对话" }).closest("section") as HTMLElement;
    expect(within(conversationPage).queryByText(/Skill 进化/)).toBeNull();
  ...
  it("uses project workspace chosen at conversation creation with run sandbox choices", async () => {
    ...
    await user.type(within(projectDialog).getByLabelText("共享工作区名称"), "app");
    await user.click(within(projectDialog).getByRole("button", { name: "创建项目" }));
    const newConversation = screen.getByRole("dialog", { name: "新建会话" });
    expect((within(newConversation).getByLabelText("所属项目") as HTMLSelectElement).value).toBe("mofang-agent");
  ...
  it("keeps step generated downloads inside the producing agent drawer", async () => {
    ...
    await user.click(await openWorkbenchProcessButton(user, stream, /文案生成 输出：生成交付文档/));
  ...
  it("shows workspace files inside the agent workbench file window with text preview", async () => {
    ...
    await user.click(within(fileWindow).getByRole("button", { name: /src\/app\.py/ }));
    const filePreview = within(workbench).getByRole("region", { name: "关联文件预览" });
    expect(await within(filePreview).findByText("print('hello from workspace')")).not.toBeNull();
    expect(within(filePreview).getByRole("button", { name: "下载 app.py" })).not.toBeNull();
  });
  ...
  it("opens a workspace file preview from the producing agent action row", async () => {
    ...
    const actionWorkspace = within(workbench).getByRole("region", { name: "动作与文件工作区" });
    const filePreview = within(actionWorkspace).getByRole("region", { name: "关联文件预览" });
    expect(within(workbench).getByRole("button", { name: "动作与文件" }).getAttribute("aria-pressed")).toBe("true");
  ...
  it("shows Codex-style chat replies with Kimi-style inline cluster actions", async () => {
    ...
    expect(within(stream).queryByRole("status", { name: /Agent 集群/ })).toBeNull();
    expect(within(stream).queryByRole("button", { name: /生成了结果/ })).toBeNull();
    expect(within(stream).queryByRole("button", { name: /文案生成 调用模型：qwen-max/ })).toBeNull();
    expect(within(stream).queryByRole("button", { name: /文案生成 输出：得到一版可拍摄脚本文案/ })).toBeNull();
    ...
    expect(within(workbenchDrawer).getByRole("region", { name: "调度简报" })).not.toBeNull();
    await openWorkbenchView(user, workbenchDrawer, "助手总览");
    await user.click(within(workbenchDrawer).getByRole("button", { name: /打开费曼 · 文案工作调度/ }));
  ...
  it("shows recruited subagents from the main agent plan", async () => {
    ...
    const copywriterChip = within(stream).getByRole("button", { name: /打开 费曼 · 文案 调度详情/ });
    ...
    expect(within(panel).getAllByText("费曼 · 文案").length).toBeGreaterThan(0);
    expect(within(panel).getByText("qwen-max")).not.toBeNull();
    ...
    expect(within(panel).getAllByText("苏澈 · 审查").length).toBeGreaterThan(0);
    expect(within(panel).getByText("deepseek-chat")).not.toBeNull();
  ...
  it("keeps recruited subagent statuses isolated when planned steps are shared", async () => {
    ...
    const betaChip = within(stream).getByRole("button", { name: /打开 夏衡 · 验证 调度详情/ });
    expect(within(betaChip).getByText(/已安排/)).not.toBeNull();
    expect(within(betaChip).getByText("验证功能与结果")).not.toBeNull();
    ...
    const alphaCard = within(panel).getByRole("button", { name: /打开韩序 · 规划工作调度/ });
    const betaCard = within(panel).getByRole("button", { name: /打开夏衡 · 验证工作调度/ });
    const gammaCard = within(panel).getByRole("button", { name: /打开谢安 · 修复工作调度/ });
  ...
  it("surfaces harness execution profile in the routing process details", async () => {
    ...
    const stream = screen.getByRole("region", { name: "主对话内容" });
    await user.click(await openWorkbenchProcessButton(user, stream, /主 Agent 选择派单/));
  ...
  it("renders harness lifecycle events as explicit process cards", async () => {
    ...
    const stream = screen.getByRole("region", { name: "主对话内容" });
    const harnessCard = await openWorkbenchProcessButton(user, stream, /Harness 启动：vibe_engineer \/ openai/);
  ...
  it("renders tool requested events from argument summaries without argument values", async () => {
    ...
    const stream = screen.getByRole("region", { name: "主对话内容" });
    const toolCard = await openWorkbenchProcessButton(user, stream, /工具请求：读取文件/);
  ...
  it("renders model stream progress without exposing legacy raw delta text", async () => {
    ...
    const stream = screen.getByRole("region", { name: "主对话内容" });
    const reasoningCard = await openWorkbenchProcessButton(user, stream, /思考过程：模型正在分析/);
    ...
    await user.click(await openWorkbenchProcessButton(user, stream, /输出进度：模型正在生成/));
  ...
  it("renders tool progress rows from safe metadata without raw command or output text", async () => {
    ...
    await user.click(await openWorkbenchProcessButton(user, stream, /文案生成 编辑文件失败/));
  ...
  it("renders agent process steps as an ordered timeline with concrete per-step details", async () => {
    ...
    await openWorkbenchView(user, workbenchDrawer, "调度讨论");
    expect(within(workbenchDrawer).getByRole("region", { name: "调度简报" })).not.toBeNull();
    const brief = within(workbenchDrawer).getByRole("region", { name: "调度简报" });
    expect(within(brief).getByText("成员发言")).not.toBeNull();
    expect(within(brief).getByRole("button", { name: /文案生成意见 文案建议主打灯谜游园会/ })).not.toBeNull();
    expect(within(brief).getByRole("button", { name: /分歧 是否保留嘉宾签到环节存在分歧/ })).not.toBeNull();
    expect(within(brief).getByRole("button", { name: /主 Agent 裁决 主 Agent 采纳灯谜游园会方案/ })).not.toBeNull();
    const coordinationActions = within(brief);
    const mainPlan = coordinationActions.getByRole("button", { name: /调度判断 主 Agent|主 Agent 接收任务：选择运行模式、角色和模型/ });
    const dispatch = coordinationActions.getByRole("button", { name: /主 Agent 派单给文案生成、导演|派工/ });
    const minutes = coordinationActions.getByRole("button", {
      name: /主 Agent 裁决|讨论纪要：共识 采用灯谜游园会，压缩签到流程；分歧 是否保留嘉宾签到/,
    });
    ...
    const planDecision = await openProcessDetailGroup(user, planDrawer, "决策");
    expect(within(planDecision).getAllByText(/派单|混合/).length).toBeGreaterThan(0);
    await user.click(within(planDecision).getByRole("button", { name: "关闭" }));
    const planActivity = await openProcessDetailGroup(user, planDrawer, "活动");
    expect(within(planActivity).getAllByText(/导演|文案生成|剪辑师/).length).toBeGreaterThan(0);
    await user.click(within(planActivity).getByRole("button", { name: "关闭" }));
    ...
    await user.click(within(outputWorkbench).getByRole("button", { name: /打开费曼 · 文案工作调度/ }));
    expect(within(outputWorkbench).getByRole("region", { name: "费曼 · 文案工作调度" })).not.toBeNull();
    ...
    await user.click(within(minutesWorkbench).getByRole("button", { name: "返回调度总览" }));
  ...
  it("compresses long agent workbench traces until the operator expands them", async () => {
    ...
    await user.click(within(stream).getByRole("button", { name: /Agent 工作席/ }));
    const workbenchDrawer = await screen.findByRole("dialog", { name: "Agent 工作席详情" });
    expect(within(workbenchDrawer).queryByRole("region", { name: "重点摘要" })).toBeNull();
    const writerCard = within(workbenchDrawer).getByRole("button", { name: /打开顾准 · 文案工作调度/ });
    expect(within(writerCard).getByText(/17 条，点击查看/)).not.toBeNull();
    expect(within(writerCard).queryByText(/阶段 01 完成/)).toBeNull();

    await user.click(writerCard);
    const actions = within(workbenchDrawer).getByRole("region", { name: "顾准 · 文案工作调度" });
    expect(within(actions).getByText(/已折叠 \d+ 个较早动作/)).not.toBeNull();
    expect(within(actions).queryByText(/阶段 01 完成/)).toBeNull();
    expect(within(actions).getByText(/阶段 16 完成/)).not.toBeNull();

    await user.click(within(actions).getByRole("button", { name: /显示全部动作/ }));
  ...
  it("uses backend safe timeline summaries instead of raw event payload text", async () => {
    ...
    await user.click(screen.getByRole("button", { name: conversationOpenButtonName }));
    const stream = screen.getByRole("region", { name: "主对话内容" });
    const reviewCard = await openWorkbenchProcessButton(user, stream, /reviewer completed review/);

    expect(stream.textContent).not.toContain("reviewer completed review");
  ...
  it("aggregates model delta chunks in the conversation process summary", async () => {
    ...
    const reasoningCard = await openWorkbenchProcessButton(user, stream, /思考过程：模型正在分析/);
    const workbench = await openAgentWorkbench(user, stream);
    const textCard = within(workbench).getByRole("button", { name: /输出进度：模型正在生成/ });
  ...
  it("shows planned task chain status from safe run events", async () => {
    ...
    expect(within(chain).getByText("许砚 · 实现")).not.toBeNull();
    ...
    expect(within(chain).getByText("苏澈 · 审查")).not.toBeNull();
    ...
    expect(within(chain).getByText("谢安 · 汇总")).not.toBeNull();
  ...
  it("summarizes Vibe engineer run posture from real process events", async () => {
    ...
    expect(within(stream).queryByRole("status", { name: /任务态势/ })).toBeNull();
    expect(within(stream).queryByRole("status", { name: /Agent 集群/ })).toBeNull();
    expect(within(stream).getByRole("button", { name: /Agent 工作席/ })).not.toBeNull();
  ...
  it("uses ordered artifacts for process rows instead of vague generated-result summaries", async () => {
    ...
    const directorOutput = within(workbench).getByRole("button", {
  ...
  it("falls back to concrete artifact titles when upstream artifact text is generic", async () => {
    ...
    await user.click(screen.getByRole("button", { name: conversationOpenButtonName }));
    const stream = screen.getByRole("region", { name: "主对话内容" });
    const outputRow = await openWorkbenchProcessButton(user, stream, /文案生成 输出：中秋活动文案初稿/);
  ...
  it("locks page scrolling while the process drawer is open and closes from the backdrop", async () => {
    ...
    const stream = screen.getByRole("region", { name: "主对话内容" });
    await user.click(
      await openWorkbenchProcessButton(user, stream, /文案生成 输出：得到一版可拍摄脚本文案/),
    );
  ...
  it("refreshes process drawer content while viewing a terminal run", async () => {
    ...
    const stream = screen.getByRole("region", { name: "主对话内容" });
    await user.click(await openWorkbenchProcessButton(user, stream, /文案生成 输出：第一版摘要/));
    const drawer = await screen.findByRole("dialog", { name: "运行过程详情" });
    expect(within(drawer).getByRole("button", { name: /产物/ })).not.toBeNull();
  ...
  it("returns to the agent workbench when closing a nested process drawer", async () => {
    const user = userEvent.setup();
    ...
    await openWorkbenchView(user, workbenchDrawer, "调度讨论");
    await user.click(within(workbenchDrawer).getByRole("button", { name: /调度判断 主 Agent|主 Agent 接收任务：main_agent_plan/ }));
    const processDrawer = await screen.findByRole("dialog", { name: "运行过程详情" });

    await user.click(within(processDrawer).getByRole("button", { name: "关闭" }));

    await waitFor(() => expect(screen.queryByRole("dialog", { name: "运行过程详情" })).toBeNull());
    expect(screen.getByRole("dialog", { name: "Agent 工作席详情" })).not.toBeNull();
    await user.click(within(workbenchDrawer).getByRole("button", { name: "关闭" }));
    await waitFor(() => expect(screen.queryByRole("dialog", { name: "Agent 工作席详情" })).toBeNull());
  });
  ...
  it("refreshes newly recruited agents without a manual page reload", async () => {
    ...
    await user.click(within(stream).getByRole("button", { name: /Agent 工作席 1 个 Agent/ }));
    const workbenchDrawer = await screen.findByRole("dialog", { name: "Agent 工作席详情" });
    await waitFor(() => expect(within(workbenchDrawer).getAllByText("费曼 · 文案").length).toBeGreaterThan(0));
    expect(within(workbenchDrawer).queryByText("黎初 · 导演")).toBeNull();
    ...
    await waitFor(() => expect(within(drawer).getByText("主 Agent 选择派单：导演、文案生成、剪辑师")).not.toBeNull(), {
      timeout: 2500,
    });
    const activityDetail = await openProcessDetailGroup(user, drawer, "活动");
    expect(within(activityDetail).getByText("导演、文案生成、剪辑师")).not.toBeNull();
    ...
    await waitFor(() => expect(within(refreshedWorkbenchDrawer).getAllByText("黎初 · 导演").length).toBeGreaterThan(0), {
      timeout: 2500,
    });
  ...
  it("refreshes new process cards while a different process detail drawer stays open", async () => {
    ...
    const stream = screen.getByRole("region", { name: "主对话内容" });
    await user.click(await openWorkbenchProcessButton(user, stream, /文案生成 输出：第一张过程卡/));
    const drawer = await screen.findByRole("dialog", { name: "运行过程详情" });
  ...
  it("shows localized process summaries with participating roles instead of raw event codes", async () => {
    ...
    const stream = screen.getByRole("region", { name: "主对话内容" });
    await user.click(await openWorkbenchProcessButton(user, stream, /结论 主 Agent|主 Agent 裁决/));
    const drawer = await screen.findByRole("dialog", { name: "运行过程详情" });
  ...
  it("keeps workbench dispatch discussion in overview and execution actions in the action file view", async () => {
    const user = userEvent.setup();
    render(<TestApp initialPath="/" />);
    ...
    expect(within(workbench).getByRole("button", { name: "动作与文件" })).not.toBeNull();
    expect(within(workbench).queryByRole("button", { name: "调度讨论" })).toBeNull();
    expect(within(workbench).queryByRole("button", { name: "实际动作" })).toBeNull();
    expect(within(workbench).getByRole("region", { name: "调度简报" })).not.toBeNull();
    expect(within(workbench).getByRole("button", { name: /结论 主 Agent/ })).not.toBeNull();
    expect(within(workbench).queryByRole("button", { name: /文案生成 输出/ })).toBeNull();

    await user.click(within(workbench).getByRole("button", { name: "动作与文件" }));
    expect(within(workbench).getByRole("button", { name: /文案生成 输出/ })).not.toBeNull();
    expect(within(workbench).queryByRole("button", { name: /结论 主 Agent/ })).toBeNull();
  });
  ...
  it("keeps one new-conversation entry and collects project metadata before creating", async () => {
    ...
    await user.type(within(projectDialog).getByLabelText("项目 ID"), "browser-tools");
    await user.clear(within(projectDialog).getByLabelText("共享工作区名称"));
    await user.type(within(projectDialog).getByLabelText("共享工作区名称"), "plugin");
    await user.click(within(projectDialog).getByRole("button", { name: "创建项目" }));
    const dialog = screen.getByRole("dialog", { name: "新建会话" });
    expect(within(dialog).queryByLabelText("共享工作区名称")).toBeNull();
    await user.type(within(dialog).getByLabelText("会话标题"), "浏览器插件开发");
    expect((within(dialog).getByLabelText("所属项目") as HTMLSelectElement).value).toBe("browser-tools");
  ...
  it("starts an empty tenant with project setup before conversation setup", async () => {
    visibleProjectWorkspaces = [];
    visibleRunListItems = [];
    visibleConversationRuns = [];
  ...
  it("starts a tenant with an existing project and no conversations at conversation setup", async () => {
    visibleProjectWorkspaces = [
      {
        project_id: "browser-tools",
        label: "浏览器工具",
    ...
    expect((within(dialog).getByLabelText("所属项目") as HTMLSelectElement).value).toBe("browser-tools");
    expect(screen.queryByRole("dialog", { name: "新建项目工作区" })).toBeNull();
    expect(screen.getByText("请选择项目并创建第一条会话。")).not.toBeNull();
  });

  it("closes project and run settings dialogs from the backdrop or Escape without closing from dialog content", async () => {
    const user = userEvent.setup();
    render(<TestApp initialPath="/" />);
    ...
    expect(screen.queryByRole("dialog", { name: "新建项目工作区" })).toBeNull();
    await waitFor(() => expect(document.activeElement).toBe(projectTrigger));
    ...
    expect(screen.getByRole("dialog", { name: "本次运行设置" })).not.toBeNull();
    await user.keyboard("{Escape}");
    expect(screen.queryByRole("dialog", { name: "本次运行设置" })).toBeNull();
    expect(document.body.style.overflow).toBe("");
  ...
  it("rejects absolute or traversing workspace paths in the new-project form", async () => {
    ...
    expect((within(dialog).getByRole("button", { name: "创建项目" }) as HTMLButtonElement).disabled).toBe(true);
  ...
  it("keeps a clear mobile hierarchy for chat sessions, content, and run settings", async () => {
    ...
    expect(screen.getByText("详细设置")).not.toBeNull();
  });
  ...
  it("keeps the new chat controls compact while exposing pre-run sandbox settings", async () => {
    ...
    await user.click(within(screen.getByRole("dialog", { name: "本次运行设置" })).getByRole("button", { name: "关闭运行设置" }));
    expect(screen.queryByRole("dialog", { name: "本次运行设置" })).toBeNull();
    expect(screen.getByRole("region", { name: "主对话内容" })).not.toBeNull();
  });
  ...
  it("manages HTTP JSON plugin resources from the MCP page", async () => {
    ...
    expect(screen.getByText("adapter package requires plugin approval before activation")).not.toBeNull();
  ...
  it("uses adapter descriptors to choose capability adapter defaults", async () => {
    ...
    await user.type(screen.getByLabelText("资源字段 workflow_id"), "daily_report");
  ...
  it("renders simple descriptor resource fields and saves them as plugin resource config", async () => {
    const user = userEvent.setup();
    ...
    const workflowField = screen.getByLabelText("资源字段 workflow_id");
    expect(workflowField.hasAttribute("required")).toBe(true);
    await user.clear(workflowField);
    await user.type(workflowField, "nightly_check");
    await user.clear(screen.getByLabelText("插件 ID"));
    await user.type(screen.getByLabelText("插件 ID"), "workflow-plugin");
    await user.clear(screen.getByLabelText("插件名称"));
    await user.type(screen.getByLabelText("插件名称"), "Workflow Plugin");
    await user.clear(screen.getByLabelText("能力 ID 1"));
    await user.type(screen.getByLabelText("能力 ID 1"), "workflow.daily");
    await user.click(screen.getByRole("button", { name: "保存插件" }));
      ...
      expect(
        requests.find(
          (request) =>
            request.path === "/api/v1/admin/plugins" &&
            request.method === "POST" &&
            (request.body as { id?: string }).id === "workflow-plugin",
        ),
      ).toBeTruthy(),
    );
    expect(
      ...
      resource_config: { workflow_id: "nightly_check" },
      capabilities: [
        {
          id: "workflow.daily",
          adapter: "workflow",
  ...
  it("does not leak descriptor resource fields after switching plugin adapters", async () => {
    const user = userEvent.setup();
    ...
    expect(await screen.findByRole("heading", { name: "插件适配器目录" })).not.toBeNull();
    await user.selectOptions(screen.getByLabelText("能力适配器 1"), "workflow");
    await user.type(screen.getByLabelText("资源字段 workflow_id"), "nightly_check");
    await user.selectOptions(screen.getByLabelText("能力适配器 1"), "http_json");

    expect(screen.queryByLabelText("资源字段 workflow_id")).toBeNull();
    await user.clear(screen.getByLabelText("插件 ID"));
    await user.type(screen.getByLabelText("插件 ID"), "http-plugin");
    await user.clear(screen.getByLabelText("插件名称"));
    await user.type(screen.getByLabelText("插件名称"), "HTTP Plugin");
    await user.click(screen.getByRole("button", { name: "保存插件" }));
      ...
      expect(
        requests.find(
          (request) =>
            request.path === "/api/v1/admin/plugins" &&
            request.method === "POST" &&
            (request.body as { id?: string }).id === "http-plugin",
        ),
      ).toBeTruthy(),
    );
    expect(
  ...
  it("uses the union of capability adapter resource fields", async () => {
    const user = userEvent.setup();
    ...
    await user.type(screen.getByLabelText("资源字段 workflow_id"), "nightly_check");
    await user.clear(screen.getByLabelText("插件 ID"));
    await user.type(screen.getByLabelText("插件 ID"), "mixed-plugin");
    await user.clear(screen.getByLabelText("插件名称"));
    await user.type(screen.getByLabelText("插件名称"), "Mixed Plugin");
    await user.clear(screen.getByLabelText("能力 ID 1"));
    await user.type(screen.getByLabelText("能力 ID 1"), "search.query");
    await user.type(screen.getByLabelText("能力 ID 2"), "workflow.daily");
    await user.click(screen.getByRole("button", { name: "保存插件" }));
      ...
      expect(
        requests.find(
          (request) =>
            request.path === "/api/v1/admin/plugins" &&
            request.method === "POST" &&
            (request.body as { id?: string }).id === "mixed-plugin",
        ),
      ).toBeTruthy(),
    );
    expect(
  ...
  it("blocks invalid descriptor number fields before saving a plugin", async () => {
    const user = userEvent.setup();
    ...
    await user.selectOptions(screen.getByLabelText("能力适配器 1"), "workflow");
    await user.type(screen.getByLabelText("资源字段 workflow_id"), "nightly_check");
    await user.type(screen.getByLabelText("资源字段 retry_limit"), "abc");
    await user.click(screen.getByRole("button", { name: "保存插件" }));

    const alert = await screen.findByRole("alert");
    expect(alert.textContent).toContain("资源字段 retry_limit 必须是数字");
    expect(
      requests.find(
  ...
  it("blocks unsupported required descriptor resource fields before saving a plugin", async () => {
    const user = userEvent.setup();
    visiblePluginAdapterDescriptors = [
      {
        ...pluginAdapterDescriptors[1],
    ...
    const alert = await screen.findByRole("alert");
    expect(alert.textContent).toContain("资源字段 workflow_options 暂不支持在表单中配置");
    expect(
      requests.find(
        (request) => request.path === "/api/v1/admin/plugins" && request.method === "POST",
      ),
