...
def _architecture_plan_markdown(*, title: str, request: str) -> str:
    return "\n".join(
        (
            ...
            "",
            "## 阶段自修复闭环",
            "- 阶段验证失败时，先定位失败阶段、触发条件、最小复现和影响边界，再决定修复动作。",
            "- 修复必须限制在已批准架构和阶段修改范围内；修复后重跑该阶段验证和受影响主流程验证。",
            "- 子 Agent/实现角色必须用 `stage_repair_actions` 记录已执行的诊断、修复、重测和无法自动修复的原因。",
            "- 无法在边界内修复时，输出阻塞原因、需要的外部输入和可安全继续的下一阶段，不允许静默降级。",
            "",
            "## 验收矩阵",
