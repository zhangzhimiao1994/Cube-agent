...
def _safe_provider_string(value: object) -> str | None:
    if isinstance(value, str) and _SAFE_PROVIDER_VALUE.fullmatch(value) is not None:
        return value
    return None
    ...
    for raw_call in raw_calls:
        ...
        try:
            ...
            if not isinstance(frozen, Mapping):
                raise TypeError
            parsed.append(ToolCall(id=identifier, name=name, arguments=frozen))
...
def _metadata(response: object, choice: object) -> Mapping[str, JsonScalar]:
    metadata: dict[str, JsonScalar] = {}
    request_id = _safe_provider_string(_attribute(response, "_request_id"))
    if request_id is None:
        request_id = _safe_provider_string(_attribute(response, "id"))
    ...
    for output_name, attribute_name in (
        ...
        safe_value = _safe_provider_string(_attribute(response, attribute_name))
    ...
    finish_reason = _safe_provider_string(_attribute(choice, "finish_reason"))
    if finish_reason is not None:
        metadata["finish_reason"] = finish_reason
    return metadata


def _parse_response(response: object, deployment_id: str) -> ModelResponse:
    ...
    return ModelResponse(
        ...
        provider_metadata=_metadata(response, choice),
    )


def _transport_error(deployment_id: str, error: Exception) -> ModelTransportError:
    details: list[str] = []
    status = _attribute(error, "status_code")
    if isinstance(status, int) and not isinstance(status, bool) and 100 <= status <= 599:
        details.append(f"status={status}")
    request_id = _safe_provider_string(_attribute(error, "request_id"))
    if request_id is not None:
    ...
    async def invoke(
        ...
        *,
        ...
        try:
            ...
            parsed = _parse_response(response, deployment.id)
        except asyncio.CancelledError:
            ...
            await _close_after_failure(client)
            raise _transport_error(deployment.id, error) from None
