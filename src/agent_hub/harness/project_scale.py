...
def _capability_message(case: ProjectScaleCase) -> str:
    requirements = {
        ...
        "medium": (
            ...
            "POST /tenants/:tenant_id/opportunities, GET /tenants/:tenant_id/opportunities, "
            "and PATCH /tenants/:tenant_id/opportunities/:id; "
            "POST /tenants/:tenant_id/reminders and GET /tenants/:tenant_id/reminders. "
            "The opportunities list endpoint is required and must persist created/patched "
            "opportunities across process restart. Cross-tenant references and missing ids must "
            "return 404 with {error:{code,message}}. For create/update requests with reference "
            "fields, resolve account_id/contact_id inside the URL tenant before validating "
            "unrelated fields such as email, due_at, note, amount, or stage; a foreign or missing "
            "reference must return 404 NOT_FOUND even when other required fields are absent. "
